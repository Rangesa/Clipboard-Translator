...
- **新ホットキーシステム (Low-level Hook)**: Windows API (`SetWindowsHookExW`) を使用した低レイヤーのキーフックを実装。より正確で応答性の高いホットキー検知が可能になりました。
- **通知システム**: 翻訳エラー時などに Windows のトースト通知を表示する機能を追加。
- **自動リリース (CI/CD)**: GitHub Actions を導入。バージョンタグをプッシュするだけで自動的にリリースビルド (`.exe`) が作成・公開されるようになりました。

### Changed

- **UIの改善**: 翻訳結果ウィンドウのレスポンス向上と表示の微調整。
- **コードのリファクタリング**: `main.rs` および `ui/result.rs` のロジックを整理し、安定性を向上。
- **ファイル構成の整理**: スクリプト類をルートに配置し直し、アクセシビリティを改善。

## [0.1.2] - 2026-01-29

### Added

- **GUIからの自動起動設定**: 設定画面（`--setup`）に「Windows起動時に自動で起動する」チェックボックスを追加。コマンドラインを使わずに設定が可能になりました。
